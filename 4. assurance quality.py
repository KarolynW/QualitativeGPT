...
file_path = 'combined_thread_messages.xlsx'
output_file_path = 'filtered_combined_thread_messages_with_ai_responses.xlsx'
...
# Check each combined message and store the AI's response
...
if 'Retain' not in combined_data.columns:
    combined_data['Retain'] = None
if 'Reason' not in combined_data.columns:
    combined_data['Reason'] = None

for index, row in combined_data.iterrows():
    if pd.isna(row['Retain']):
        if index % 10 == 0:
            print(Fore.YELLOW + f"Processing row {index + 1} of {total_rows}" + Style.RESET_ALL)
        ai_response = check_message(row['Combined Messages'])
        try:
            response_json = json.loads(ai_response)
            combined_data.at[index, 'Retain'] = response_json.get('retain')
            combined_data.at[index, 'Reason'] = response_json.get('reason')
        except json.JSONDecodeError as e:
            print(Fore.RED + f"Error parsing JSON response: {e}" + Style.RESET_ALL)
            combined_data.at[index, 'Retain'] = False
            combined_data.at[index, 'Reason'] = "Error parsing JSON response"
